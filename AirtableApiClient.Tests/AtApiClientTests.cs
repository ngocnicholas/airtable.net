...
namespace AirtableApiClient.Tests
{
    ...
    public class AtApiClientTests
    {
        ...
        public async Task TaAtApiClientListRecordsTest_Cancel()
        {
            ...
            //  (2) Replace all ' \"' with '\"'
            //  (3) Replace all '\n'  with  '\\n'
            //  (4) Remove extra spaces infront of the first ' {' front the beginning of the line and after the last the '} ' at the end of the line
        ...
        public async Task TzDAtApiClientTooManyRecordsErrorTest()
        {
            ...
            List<AirtableRecord> doubleList = new List<AirtableRecord>();
            foreach (var record in response.Records)
            {
                if (record != null)
                { 
        ...
        //----------------------------------------------------------------------------
        [TestMethod]
        //public async Task TzJAtApiClientCreateWebhooks_Cancel(CancellationTokenSource tokenSrc)
        public async Task TzJAtApiClientCreateWebhooks_Cancel()
        {
            ...
            AirtableCreateWebhookResponse? response = null;
            try
            {
                response = await task;
                Assert.IsFalse(response.Success);
        ...
        private async Task<AirtableRecord[]> GetRecordsWithFormula()
        {
            ...
            AirtableRecord[] nonNullRecords = response.Records.Where(r => r != null).ToArray();
