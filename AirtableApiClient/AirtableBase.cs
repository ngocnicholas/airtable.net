...
public class AirtableBase : IDisposable
{
	...
	private readonly JsonSerializerOptions JsonOptionIgnoreNullValues = new() { DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull, };

	public bool ShouldNotRetryIfRateLimited
	{
		get => httpClientWithRetries.ShouldNotRetryIfRateLimited;
		set => httpClientWithRetries.ShouldNotRetryIfRateLimited = value;
	}

	public int RetryDelayMillisecondsIfRateLimited
	{
		get => httpClientWithRetries.RetryDelayMillisecondsIfRateLimited;
		...
		DelegatingHandler delegatingHandler)
	{
		...
		{
			throw new ArgumentException("api Key or access token cannot be null", nameof(apiKeyOrAccessToken));
		}
	...
	{
		const string uriStr = "https://api.airtable.com/v0/meta/whoami";
		HttpRequestMessage request = new(HttpMethod.Get, uriStr);
		...
		string id)
	{
		...
		string responseBody = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
		AirtableRecord airtableRecord = JsonSerializer.Deserialize<AirtableRecord>(responseBody, JsonOptionIgnoreNullValues);
		...
		string id)
	{
		...
		string uriStr = UrlHead + Uri.EscapeDataString(tableIdOrName) + "/" + id;
		HttpRequestMessage request = new(HttpMethod.Delete, uriStr);
		HttpResponseMessage response = await httpClientWithRetries.SendAsync(request).ConfigureAwait(false);
		AirtableApiException error = await CheckForAirtableException(response).ConfigureAwait(false);
		if (error != null)
		{
		...
		string responseBody = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
		AirtableDeletedRecord deletedRecord = JsonSerializer.Deserialize<AirtableDeletedRecord>(responseBody);
		...
		bool typecast = false)
	{
		...
		string json = JsonSerializer.Serialize(new { records = fields, typecast }, JsonOptionIgnoreNullValues);
		...
		bool typecast = false)
	{
		...
		List<Fields> fieldsList = new();       // Must use List<> so that we can expand it later.
		for (int index = 0; index < records.Length; index++)
		{
			...
			{
				string json = JsonSerializer.Serialize(new { records = fieldsList.ToArray(), typecast }, JsonOptionIgnoreNullValues);
	...
	{
		string json = ReplaceUpdateMultipleRecordsInternal(idFields, typecast, returnFieldsByFieldId, performUpsert);
		...
		int? pageSize = null)
	{
		...
		UriBuilder uriBuilder = new(UrlHead + Uri.EscapeDataString(tableIdOrName) + "/" + recordId + "/comments");
		...
		if (pageSize != null)
		{
			if (pageSize is <= 0 or > MAX_PAGE_SIZE)
			{
		...
		string responseBody = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
		CommentList commentList = JsonSerializer.Deserialize<CommentList>(responseBody, JsonOptionIgnoreNullValues);
	   ...
	   string rowCommentId)
	{
		...
		{
			throw new ArgumentException("Comment ID cannot be null.", nameof(rowCommentId));
		}
		...
		string rowCommentId)
	{
		...
		string responseBody = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
		AirtableDeletedRecord deletedComment = JsonSerializer.Deserialize<AirtableDeletedRecord>(responseBody);
	...
	//
	//----------------------------------------------------------------------------
	[SuppressMessage("Usage", "CA1816:Dispose methods should call SuppressFinalize", Justification = "Unknown if it is safe to change this.")]
	public void Dispose()
	{
	...
	//
	//----------------------------------------------------------------------------
	private static string BuildParametersForListRecords(
		string offset,
		...
		)
	{
		...
		ListRecordsParameters listRecordsParameters = new()
		{
			Offset = offset,
	...
	private Uri BuildUriForListRecords(
		...
		bool returnFieldsByFieldId)
	{
		UriBuilder uriBuilder = new(UrlHead + Uri.EscapeDataString(tableIdOrName));
	...
	//
	//----------------------------------------------------------------------------
	private static void AddParametersToQuery(ref UriBuilder uriBuilder, string queryToAppend)
	{
		uriBuilder.Query = uriBuilder.Query?.Length > 1 ? uriBuilder.Query[1..] + "&" + queryToAppend : queryToAppend;
	}
		...
		bool typecast = false)
	{
		...
		{
			throw new ArgumentException("Table ID or Name cannot be null", nameof(tableIdOrName));
		}
		...
		string json = JsonSerializer.Serialize(fieldsAndTypecast, JsonOptionIgnoreNullValues);
		HttpRequestMessage request = new(httpMethod, uriStr)
		{
		...
		string responseBody = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
		AirtableRecord airtableRecord = JsonSerializer.Deserialize<AirtableRecord>(responseBody);
	...
	private async Task<AirtableCreateUpdateReplaceMultipleRecordsResponse> CreateUpdateReplaceMultipleRecords(string tableIdOrName, HttpMethod method, string json)
	{
		...
		string uriStr = UrlHead + Uri.EscapeDataString(tableIdOrName) + "/";
		HttpRequestMessage request = new(method, uriStr)
		{
			Content = new StringContent(json, Encoding.UTF8, "application/json")
		};
		HttpResponseMessage response = await httpClientWithRetries.SendAsync(request).ConfigureAwait(false);
		...
		string responseBody = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
		AirtableRecordList recordList = JsonSerializer.Deserialize<AirtableRecordList>(responseBody);
	...
	//
	//----------------------------------------------------------------------------
	private static IdFields[] ConvertAirtableRecordsToIdFields(AirtableRecord[] records, bool upSert = false)
	{
		if (records == null || records.Length == 0 || records.Length > MAX_RECORD_OPERATION_SIZE)
		{
			throw new ArgumentException("Record list cannot be null or empty or cannot have more than 10 records");
		...
		for (int index = 0; index < records.Length; index++)
		{
			...
			{
				throw new ArgumentException("All records in record list must contain an ID", nameof(records));
			}
		...
		PerformUpsert performUpsert = null)
	{
		...
		UpsertRecordsParameters upsertRecordsParameters = new()
		{
		...
		if (performUpsert != null && ((performUpsert.FieldsToMergeOn?.Length < 1) || performUpsert.FieldsToMergeOn.Length > 3))
		{
	...
	//
	//----------------------------------------------------------------------------
	private static async Task<AirtableApiException> CheckForAirtableException(HttpResponseMessage response)
	{
		switch (response.StatusCode)
		{
			...
			case (System.Net.HttpStatusCode)422:    // There is no HttpStatusCode.InvalidRequest defined in HttpStatusCode Enumeration.
				string detailedErrorMessage = await ReadResponseErrorMessage(response).ConfigureAwait(false);
	...
	private static async Task<string> ReadResponseErrorMessage(HttpResponseMessage response)
	{
		...
		if (json.MessagePart?.ErrorType != null)
		{
		...
		)
	{
		...
		{
			Uri uri = BuildUriForListRecords(tableIdOrName, offset, fields, filterByFormula, maxRecords, pageSize, sort, view, cellFormat, timeZone, userLocale, returnFieldsByFieldId);
	   ...
	   string rowCommentId = null)      // == null if we are doing Create Comment Otherwise we are doing Update Comment
	{
		...
		HttpRequestMessage request = new(httpMethod, uriStr);
		string json = JsonSerializer.Serialize(new { text = commentText }, JsonOptionIgnoreNullValues);
		...
		string responseBody = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
		Comment comment = JsonSerializer.Deserialize<Comment>(responseBody, JsonOptionIgnoreNullValues);
	...
	//
	//----------------------------------------------------------------------------
	private static void TableIdOrNameAndRecordIdCheck(string tableIdOrName, string id)
	{
		if (string.IsNullOrEmpty(tableIdOrName))
		{
			throw new ArgumentException("Table ID or name cannot be null", nameof(tableIdOrName));
